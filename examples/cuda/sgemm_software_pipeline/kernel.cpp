        ...
        for (int by_i = 0; by_i < BY/SBY; ++by_i) {
                for (int bx_i = 0; bx_i < BX/SBX; ++bx_i) {
                        ...
                        for(int b_i = 0; b_i < BSG_TILE_GROUP_X_DIM; b_i ++){
                                ...
                                bsg_unroll(2)
                                for (int j = 0; j < SBZ; ++j) {
                                  float * bsg_attr_noalias col_anchor = &(mat1[sb_anchor_y * BX + 0 + j]);
                                  float * bsg_attr_noalias row_anchor = &(mat2[(0 + j) * BY + sb_anchor_x]);
                                  bsg_unroll(16)
                                  for (int i = 0; i < SBX; ++i) {
                                ...
                                // LOOP
                                for(int sbx_i = 2; sbx_i < BX; sbx_i += SBZ) {
                                  float * bsg_attr_noalias col_anchor;
                                  float * bsg_attr_noalias row_anchor;
                                  ...
                                  for(int sby_i = 0; sby_i < SBY; ++sby_i){
                                    bsg_unroll(16)
                                    for(int sbx_i = 0; sbx_i < SBX; ++sbx_i){
                                      psum[sby_i][sbx_i] = fmaf(col[0][sby_i], row[0][sbx_i], psum[sby_i][sbx_i]);
                                    }
                                  }

                                  // LOAD0
                                  col_anchor = &(mat1[sb_anchor_y * BX + sbx_i + 0]);
                                  row_anchor = &(mat2[(sbx_i + 0) * BY + sb_anchor_x]);
                                  bsg_unroll(16)
                                  for (int i = 0; i < SBX; ++i) {
                                     col[0][i] = col_anchor[i * BX];
                                     row[0][i] = row_anchor[i];
                                  }
                                  ...
                                  for(int sby_i = 0; sby_i < SBY; ++sby_i){
                                    bsg_unroll(16)
                                    for(int sbx_i = 0; sbx_i < SBX; ++sbx_i){
                                      psum[sby_i][sbx_i] = fmaf(col[1][sby_i], row[1][sbx_i], psum[sby_i][sbx_i]);
                                    }
                                  }

                                  // LOAD1
                                  col_anchor = &(mat1[sb_anchor_y * BX + sbx_i + 1]);
                                  row_anchor = &(mat2[(sbx_i + 1) * BY + sb_anchor_x]);
                                  bsg_unroll(16)
                                  for (int i = 0; i < SBX; ++i) {
                                     col[1][i] = col_anchor[i * BX];
                                     row[1][i] = row_anchor[i];
                                  }
                                }
