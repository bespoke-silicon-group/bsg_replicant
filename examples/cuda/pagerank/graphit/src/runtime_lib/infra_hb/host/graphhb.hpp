...
public:
	...
	GraphHB(const GraphHB &other) = delete;
        ...
        void calculate_blocked_index(int * block_index, int * index, int * in_neighbor, int64_t start, int64_t end, int64_t V) {
...
//            std::cout << "start c2sr calculate" << std::endl;
	    ...
	    this->calculate_blocked_index(blocked_index.data(), index.data(), _host_g.in_neighbors_shared_.get(), pod_row_start, pod_row_end, num_nodes());
