  ...
  switch(version){
    case 0: // do dense pull bfs
      for(int j = 0; j < 1; j++) //just doing one large iteration
      {
        ...
        device->enqueueJob("computeContrib_kernel", hb_mc_dimension(X,Y),{contrib_dev.getAddr(),  out_degree_dev.getAddr(), edges.num_nodes()});
        device->runJobs();
...
//        }
        device->enqueueJob("edgeset_apply_pull_serial_call", hb_mc_dimension(X,Y),{edges.getInBlockIndicesAddr(), edges.getInNeighborsAddr(), contrib_dev.getAddr(), old_rank_dev.getAddr(), edges.num_nodes(), edges.num_edges()});
        device->runJobs();
...
//        }
  ...
  int rows_within_block = (V % NUM_PODS) == 0 ? (V / NUM_PODS) : (V / NUM_PODS + 1);
            std::cout << "Simulating current pod " << CURRENT_POD << std::endl;
