...
//        }
        device->enqueueJob("edgeset_apply_pull_serial_call", hb_mc_dimension(X,Y),{edges.getInIndicesAddr(), edges.getInNeighborsAddr(), new_rank_dev.getAddr(), contrib_dev.getAddr(), rows_in_pod});
        device->runJobs();
...
//        }
        device->enqueueJob("updateVertex_kernel", hb_mc_dimension(X,Y),{old_rank_dev.getAddr(), new_rank_dev.getAddr(), error_dev.getAddr(), rows_in_pod, edges.num_edges()});
        device->runJobs();
