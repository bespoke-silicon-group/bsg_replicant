...
//      bsg_printf("Handling row %d, first is %d, last is %d, bsg_id is %d\n", d, first, last, __bsg_id);
      ...
      for(; s <= last - 16; s = s + 16) {
        ...
        register int idx12 = in_neighbors[s+12];
        register int idx13 = in_neighbors[s+13];
        register int idx14 = in_neighbors[s+14];
        register int idx15 = in_neighbors[s+15];
        ...
        register float tmp_12 = contrib[idx12];
        register float tmp_13 = contrib[idx13];
        register float tmp_14 = contrib[idx14];
        register float tmp_15 = contrib[idx15];
