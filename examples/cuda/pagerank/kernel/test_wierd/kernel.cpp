...
//    pr_dbg("d is %d, start and end is %d and %d\n", d, in_indices[d], in_indices[d+1]);
    ...
    int s = first;
    for(; s <= last - 8; s = s + 8) {
      int cidx0 = convert_idx(s, V, d);
      int cidx1 = convert_idx(s+1, V, d);
      int cidx2 = convert_idx(s+2, V, d);
      int cidx3 = convert_idx(s+3, V, d);
      int cidx4 = convert_idx(s+4, V, d);
      int cidx5 = convert_idx(s+5, V, d);
      int cidx6 = convert_idx(s+6, V, d);
      int cidx7 = convert_idx(s+7, V, d);
...
extern "C" int  __attribute__ ((noinline)) computeContrib_kernel(int * index, int* c2sr_index, float * contrib, int * out_degree, int V) {
  bsg_cuda_print_stat_start(1);
  if(bsg_id == 0) {
    c2sr_index[V] = index[V];
    bsg_printf("%d, %d, %d\n", bsg_tiles_X, bsg_tiles_Y, c2sr_index[V]);
  }
  int start, end;
  ...
  for (; start < V; start = start + cache_total) {
    ...
    for(; d <= end - 8; d = d + 8) {
      ...
      register int idx0 = index[d];
      register int idx1 = index[d+1];
      register int idx2 = index[d+2];
      register int idx3 = index[d+3];
      register int idx4 = index[d+4];
      register int idx5 = index[d+5];
      register int idx6 = index[d+6];
      register int idx7 = index[d+7];
    ...
    for(; d < end; d++) {
      ...
      register int idx = index[d];
      register float cont = one / od;
      contrib[d] = cont;
      *(c2sr_index + d) = idx;
      bsg_print_int(c2sr_index[d]);
    }
  }
...
extern "C" int  __attribute__ ((noinline)) generate_c2sr(int* c2sr_index, int * c2sr_neighbors, float * c2sr_vals, int * in_neighbors, float * contrib, int V) {
  for(size_t i = bsg_id; i < V+1; i= i+ bsg_tiles_X * bsg_tiles_Y){
    bsg_printf("Enter generate c2sr, V is %d\n", V);
    bsg_print_int(c2sr_index[i]);
  }
  bsg_cuda_print_stat_start(2);
  int32_t offset = V + 1;
  for (uint32_t k = bsg_id; k < V; k = k + bsg_tiles_X * bsg_tiles_Y) {
    int sum = 0;
    if(k < NUM_OF_SLOTS) {
      sum = 0;
    } else {
      ...
      for (; t >= 0 ; t = t - NUM_OF_SLOTS) {
        sum = sum + c2sr_index[t + 1] - c2sr_index[t];
      }
    }
    c2sr_index[offset + k] = sum;
  }
  
  barrier.sync();

  for(int i = bsg_id; i < 2*V+1; i=i+ bsg_tiles_X * bsg_tiles_Y){
    bsg_print_int(c2sr_index[i]);
  }
