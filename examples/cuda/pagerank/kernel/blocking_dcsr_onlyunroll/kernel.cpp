...
#ifdef DEBUG
#define pr_dbg(fmt, ...)                        \
    do{ if (__bsg_id == 53) bsg_printf(fmt, ##__VA_ARGS__); } while (0)
#else
#define pr_dbg(fmt, ...)
#endif
...
int edgeset_apply_pull_serial(uint32_t * bool_tag, int * dcsr_indices, int *in_indices , int *in_neighbors, float * new_rank, float * contrib, int V, int E )
{
...
//    for (int id = start + __bsg_id; id < end; id = id + bsg_tiles_X * bsg_tiles_Y) {
      for(int id = workq[b].fetch_add(GRANULARITY_PULL, std::memory_order_relaxed); id < dcsr_length; id = workq[b].fetch_add(GRANULARITY_PULL, std::memory_order_relaxed)) {
        int stop = (id + GRANULARITY_PULL) > dcsr_length ? dcsr_length : (id + GRANULARITY_PULL);
        for (int d = id; d < stop; d++) {
          int row = dcsr_indices[d];
          bsg_printf("row is %d\n", row);
          uint32_t after_mask = bool_tag[(row-start)*num+k] & mask;
          bsg_printf("Bool tag is %x and after_mask is %x\n", bool_tag[(row-start)*num+k], after_mask);
          if(after_mask) { 
            ...
            int offset = (row - start) * (KERNEL_NUM_PODS+1) + current_block;
...
extern "C" int __attribute__ ((noinline)) edgeset_apply_pull_serial_call(uint32_t * bool_tag, int * dcsr_indices, int *out_indices, int *out_neighbors, float * new_rank, float * contrib, int V, int E) {
  edgeset_apply_pull_serial(bool_tag, dcsr_indices, out_indices, out_neighbors, new_rank, contrib, V, E);
  bsg_fence();
  bsg_barrier_hw_tile_group_sync();
  return 0;
}
extern "C" int  __attribute__ ((noinline)) updateVertex_kernel(float * old_rank, float *new_rank, float * error, int V, int E) {
  ...
  align_range(rows_within_pod, &start, &end);
