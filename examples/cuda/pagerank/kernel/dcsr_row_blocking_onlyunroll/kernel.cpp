...
int edgeset_apply_pull_serial(int * dcsr_indices, int *in_indices , int *in_neighbors, float * new_rank, float * contrib, int V, int E)
{
...
//    for (int id = start + __bsg_id; id < end; id = id + bsg_tiles_X * bsg_tiles_Y) {
  for(int id = workq.fetch_add(GRANULARITY_PULL, std::memory_order_relaxed); id < dcsr_length; id = workq.fetch_add(GRANULARITY_PULL, std::memory_order_relaxed)) {
...
extern "C" int __attribute__ ((noinline)) edgeset_apply_pull_serial_call(int * dcsr_indices, int *out_indices, int *out_neighbors, float * new_rank, float * contrib, int V, int E) {
  edgeset_apply_pull_serial(dcsr_indices, out_indices, out_neighbors, new_rank, contrib, V, E);
  bsg_fence();
  bsg_barrier_hw_tile_group_sync();
  return 0;
}
