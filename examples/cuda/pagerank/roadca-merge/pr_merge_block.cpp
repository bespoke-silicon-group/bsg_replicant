...
//
  ...
  int64_t rows_in_pod = (V % NUM_PODS) == 0 ? (V / NUM_PODS) : (V / NUM_PODS + 1);
  int pod_row_start = CURRENT_POD * rows_in_pod;
//  int pod_row_end = (pod_row_start + rows_in_pod) > V ? V : (pod_row_start + rows_in_pod);
  ...
  switch(version) {
    case 0: // do dense pull bfs
      ...
      for(int j = 0; j < 1; j++) //just doing one large iteration
      {
        ...
        device->enqueueJob("pr_merge_all", hb_mc_dimension(X,Y),{edges.getInIndicesAddr(), edges.getInNeighborsAddr(), out_degree_dev.getAddr(), old_rank_dev.getAddr(), new_rank_dev.getAddr(), contrib_dev.getAddr(), contrib_new_dev.getAddr(), rows_in_pod});
        device->runJobs();
