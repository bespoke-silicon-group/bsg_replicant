...
class VanillaStatsParser:
    ...
    def __print_subblock_abs_cycles(self, stat_file):
        stat_file.write("Per-SubBlock Stats\n")
        self.__print_stat(stat_file, "start_lbreak")
        for tag in self.manycore_stat.keys():
            if(self.manycore_stat[tag]["global_ctr"] and tag >=2 and tag < 15):
                block_string = "Runtime of sub-block " + str(tag) + " = " + str(self.manycore_stat[tag]["cycle_parallel_cnt"]) + "\n"
                stat_file.write(block_string)
        self.__print_stat(stat_file, "end_lbreak")
    ...
    def print_manycore_stats_all(self):
        ...
        self.__print_subblock_abs_cycles(manycore_stats_file)
