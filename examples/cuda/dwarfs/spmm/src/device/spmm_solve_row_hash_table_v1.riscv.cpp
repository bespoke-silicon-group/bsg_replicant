...
void spmm_scalar_row_product(float Aij, int Bi)
{
    ...
    // unrolled for ilp
    int nz_base = 0;
    for (; nz_base + UNROLL < nnz; nz_base += UNROLL) {
        float Bij[UNROLL]; // val                
        float Cij[UNROLL]; // val        
        int   Bj [UNROLL]; // idx
        int   Bjh[UNROLL]; // hash(idx)
        // load vals/cols
        for (int nz = 0; nz < UNROLL; nz++) {
            Bij[nz] = vals[nz_base+nz];
            Bj [nz] = cols[nz_base+nz];
        }
        // compute hashes/values
        for (int nz = 0; nz < UNROLL; nz++) {
            Bjh[nz] = hash(Bj[nz]);
#if !defined(SPMM_NO_FLOPS)
            Cij[nz] = Bij[nz] * Aij; // fmult
#else
            Cij[nz] = Aij[nz];
#endif
        }
        // perform hash table lookup/update
        for (int nz = 0; nz < UNROLL; nz++) {
            spmm_update_table(Cij[nz], Bj[nz], Bjh[nz]);
        }
    }
    // find remaining nonzeros serially
    for (; nz_base < nnz; nz_base++) {
        int nz = nz_base;
        float Bij = vals[nz];
        int   Bj  = cols[nz];
        // stall on Bij
#if !defined(SPMM_NO_FLOPS)
        float Cij = Aij * Bij; // fmult
#else
        float Cij = Aij;
#endif
        // stall on Bj
...
void spmm_solve_row(int Ai)
{
    ...
    for (int nonzero = 0; nonzero < nnz; nonzero++) {
        ...
        spmm_scalar_row_product<2>(Aij, Bi);
    }
