...
# .FORCE is used to ensure that the <test_name>.log rules are always re-run.
.FORCE:
...
$(INDEPENDENT_LOGS): $(SIM_PATH)/%.log: $(SIM_PATH)/% %.rule #.FORCE
	$< 2>&1 +ntb_random_seed_automatic +vpdfile+$<.vpd $(SIM_ARGS)\
		+c_args="$(C_ARGS)" | tee $@

$(SIM_PATH)/%.log: $(SIM_PATH)/test_unified_main %.rule #.FORCE
	$< 2>&1 +ntb_random_seed_automatic +vpdfile+$<.vpd $(SIM_ARGS) \
...
# squeakyclean removes any outputs that can be reused between regression suites
squeakyclean: $(LIBRARIES_CLEANS) $(HARDWARE_CLEANS) 

.PHONY: regression clean squeakyclean .FORCE $(USER_RULES) $(USER_CLEAN_RULES)
