...
uint8_t *get_pkt(uint32_t addr, uint32_t data, uint8_t x, uint8_t y) {
	...
	/* byte 1 = MSB {6 LSB of data = 0s, 2 MSB of src_y} LSB */
	packet[1] = get_bits(MY_Y, 1, 2);

	/* bytes 2-4 = next 24b of data, which are 0 */
	
	/* byte 5 = {op, op_ex, 2 MSB of data = 0s} */
	packet[5] = (0xF << 2) + (OP_REMOTE_STORE << (2 + OP_EX_BIT)); 
...
uint8_t **init_vcache () {
	...
	for (int cache = 0; cache < NUM_VCACHE; cache++) {
		for (int tag = 0; tag < num_tags; tag++) {
			uint32_t addr = (1 << (ADDR_BIT-1)) | (tag << 3);  
			packets[(cache * num_tags) + tag] = get_pkt(tag, 0, cache, 5);
		}
...
void parse_elf (char *filename) { 	
	...
	for (unsigned i = 0; i < eh->e_phnum; i++) { 
		if(ph[i].p_type == PT_LOAD && ph[i].p_memsz) { 
			if (i == TEXT) {
				num_text_pkts = NUM_VCACHE * ((ph[i].p_memsz / 4) + (ph[i].p_memsz % 4)); 
				text_pkts = calloc(num_text_pkts, sizeof(uint8_t *));
				uint8_t *instructions = calloc(ph[i].p_memsz, sizeof(uint8_t));
				if (ph[i].p_filesz) { 
				...
				for (int x = 0; x < NUM_VCACHE; x++) { 
					for (int ofs = 0; ofs < ph[i].p_memsz; ofs += 4) {
						int32_t addr = (ph[i].p_paddr + ofs) >> 2;
						
						uint32_t data;
						if (ofs + 4 <= ph[i].p_memsz)
							data = *((uint32_t *) (instructions + ofs));
						else {
							for (int byte = ofs; byte < ph[i].p_memsz; byte++) {
								data += instructions[byte] <<  (byte - ofs);
							}
						}
						
						text_pkts[x * ph[i].p_memsz + ofs] = get_pkt(addr, data, x, 5);
					}
				}
			}
			
			else if (i == DATA) { /* load to tile (0, 0) */
				num_data_pkts = (ph[i].p_memsz / 4) + (ph[i].p_memsz % 4); 
				data_pkts = calloc(num_data_pkts, sizeof(uint8_t *));
				uint8_t *data_dmem = calloc(ph[i].p_memsz, sizeof(uint8_t));
				if (ph[i].p_filesz) { 
        				assert(size >= ph[i].p_offset + ph[i].p_filesz);  
					for (int byte = 0; byte < ph[i].p_filesz; byte++)
				...
				for (int ofs = 0; ofs < ph[i].p_memsz; ofs += 4) {
					uint32_t addr = (ph[i].p_paddr + ofs) >> 2;
					
					uint32_t data;
					if (ofs + 4 <= ph[i].p_memsz)
						data = *((uint32_t *) (data_dmem + ofs));
					else {
						for (int byte = ofs; byte < ph[i].p_memsz; byte++) {
							data += data_dmem[byte] <<  (byte - ofs);
						}
					}
					data_pkts[ofs] = get_pkt(addr, data, 0, 0);
				}
			}
