...
/*!
 * Runs the matrix multiplication with shared memory on a grid of 4 2x2 tile groups. 
...
int kernel_matrix_mul_shared_mem () {
	fprintf(stderr, "Running the CUDA Matrix Multiplication With Shared Memery Kernel on a grid of 4 2x2 tile groups.\n\n");
	...
	hb_mc_grid_init (&device, grid_dim_x, grid_dim_y, tg_dim_x, tg_dim_y, "kernel_matrix_mul_shared_mem", 4, argv);
