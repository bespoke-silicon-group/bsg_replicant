			     ...
			     const void *data, size_t sz)
{
	...
	for (size_t i = 0; i < n_words; i++) {
		...
		if (err != HB_MC_SUCCESS) {
			manycore_pr_err("%s: failed to send write request: %s\n",
					__func__, hb_mc_strerror(err));
			return err;
		}
	}
			    ...
			    void *data, size_t sz)
{
	...
	for (size_t i = 0; i < n_words; i++) {
		...
		if (err != HB_MC_SUCCESS) {
			manycore_pr_err("%s: failed to send read request: %s\n",
					__func__, hb_mc_strerror(err));
			return err;
		}
	}
	...
	for (size_t i = 0; i < n_words; i++) {
		...
		if (err != HB_MC_SUCCESS) {
			manycore_pr_err("%s: failed to receive read response: %s\n",
					__func__, hb_mc_strerror(err));
			return err;
		}
	}
 ...
 * @return HB_MC_FAIL if an error occured. HB_MC_SUCCESS otherwise.
 */
int hb_mc_manycore_npa_to_eva(const hb_mc_manycore_t *mc,
 ...
 * @return HB_MC_FAIL if an error occured. HB_MC_SUCCESS otherwise.
 */
int hb_mc_manycore_eva_to_npa(const hb_mc_manycore_t *mc,
