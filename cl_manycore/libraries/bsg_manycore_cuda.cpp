...
int hb_mc_tile_group_allocate (device_t *device, tile_group_t *tg){
	...
	for (int org_y = device->grid->origin_y; org_y <= (device->grid->origin_y + device->grid->dim_y - tg->dim_y); org_y++){
		for (int org_x = device->grid->origin_x; org_x <= (device->grid->origin_x + device->grid->dim_x - tg->dim_x); org_x++){
			int free = 1;
			int tile_id;
...
int hb_mc_tile_group_init (device_t* device, uint8_t dim_x, uint8_t dim_y, char* name, uint32_t argc, uint32_t argv[]) {
	...
	tg->origin_y = device->grid->origin_y;
...
int hb_mc_device_init (device_t *device, eva_id_t eva_id, char *elf, uint8_t dim_x, uint8_t dim_y, uint8_t origin_x, uint8_t origin_y) {
	...
	for (int i = 0; i < num_tiles; i++) {
		...
		if (error != HB_MC_SUCCESS)
			return HB_MC_FAIL;
		hb_mc_tile_unfreeze(device->fd, device->grid->tiles[i].x, device->grid->tiles[i].y);
	}
...
int hb_mc_device_finish (device_t *device) {
	...
	for (int i = 0; i < device->grid->dim_x * device->grid->dim_y ; i++) { /* freeze tiles */
		hb_mc_tile_freeze(device->fd, device->grid->tiles[i].x, device->grid->tiles[i].y);
	}
